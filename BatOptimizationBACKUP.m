function [model_accuracy,conMat,finalMdl,time_duration,bestKscale,bestBConstraint]=BatOptimization(fromRange,toRange)
...
ub = 1000;   % upper bound of your variables

%Plot a meshgrid for the svm [USED TO CHECK IF ALGORITHM WORKS]
% [X, Y]= meshgrid(0.1:1:10,0.1:1:10);
% Z = zeros(10);
% 
% a=0.1;
% b=0.1;
...
%     
%     for r = 1:10
%         [Z(c,r),~] = MainSVMFunction(X_train,Y_train,CV_Part,a,b);
%         a=a+1;
%     end
%     a=0.1;
%     b=b+1;
...
while iter<Max_iter
    ...
    if plot_type
        ...
        title({['BAT Optimizied SVM'], ['Optimum Z= ' num2str(gBAT(end).cost)]}, 'FontSize', 10);
        xlabel('X');
        ylabel('Y');
        cc= gca;
        cc.CameraPosition= [-15.4048  -22.5827  871.7489];        
        hold on
        s1=scatter3(xx, yy, zz, 50, 'fill', 'k');
